...
class HyperLiquid():
    ...
    """Order methods"""
    def limit_order(self, symbol, is_buy, size, price):
        try:
            order_result = self.ex.order(symbol, is_buy, size, price, {"limit": {"tif": "Gtc"}}) # Gtc = Good till cancelled
            return order_result
        except Exception as err:
            print(f"Error: {err}")
    
    def market_order(self, symbol, is_buy, size, trigger_price):
        order_result = self.ex.order(symbol, is_buy, size, trigger_price, {"trigger": {"triggerPx": trigger_price, "isMarket": True, "tpsl": "tp"}}) # Trigger price should be best price at direction
        return order_result
    
    def cancel_order(self, symbol, order_id):
        if order_id:
            try:
                cancel_result = self.ex.cancel(symbol, order_id)
